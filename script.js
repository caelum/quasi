var previousScrollPosition, scroll_state,
		info_sections = document.getElementsByClassName('article-section'),
	 	current_position = 0,
	 	didScroll = false,
		...
		titlesList = (function() {
			...
			for(var i = 0; i < info_sections.length; i++){
				var section = info_sections[i],
						title = section.getElementsByClassName("section-title")[0],
		...
		contentList = (function() {
			...
			for(var i = 0; i < info_sections.length; i++){
				list.push(
					info_sections[i].getElementsByClassName("section-content")[0].lastElementChild
				)
			}
			return list
		})(),
